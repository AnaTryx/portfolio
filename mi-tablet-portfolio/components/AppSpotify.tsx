...
const AppSpotify = ({ onClose }: AppSpotifyProps) => {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center z-[200] p-4 bg-black/40 backdrop-blur-sm">
      ...
      <div className="w-full max-w-md bg-[#121212] text-white rounded-[40px] shadow-2xl overflow-hidden flex flex-col h-[600px] animate-in slide-in-from-bottom-10 duration-500 border border-white/5">
        
        {/* Cabecera */}
        <div className="p-6 flex justify-between items-center bg-gradient-to-b from-zinc-800 to-[#121212]">
          <MoreHorizontal className="text-zinc-400 cursor-pointer" size={20} />
          <span className="text-[10px] font-bold uppercase tracking-[2px] text-zinc-400">Coding Playlist</span>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-black/40 flex items-center justify-center hover:bg-white/10 transition-colors">
        ...
        <div className="flex-1 px-10 flex flex-col items-center justify-center -mt-4">
          {/* Portada */}
          <div className="w-56 h-56 rounded-2xl shadow-[0_15px_45px_rgba(0,0,0,0.8)] mb-8 overflow-hidden">
            <img 
          ...
          {/* Info de la canción + Corazón Lucide */}
          <div className="flex items-center justify-between w-full mb-6">
            <div className="flex-1 min-w-0 pr-4">
              <h2 className="text-2xl font-bold truncate text-left">{currentTrack.title}</h2>
              <p className="text-zinc-400 text-base font-medium mt-1 text-left">{currentTrack.artist}</p>
            </div>
            <button 
            ...
            >
              <Heart 
                size={28} 
                className={`transition-colors duration-300 ${isLiked ? 'fill-[#1DB954] text-[#1DB954]' : 'text-zinc-500 group-hover:text-white'}`} 
              />
            </button>
          </div>
          ...
          <div className="w-full mb-8">
            <div className="h-1.5 w-full bg-zinc-800 rounded-full cursor-pointer group relative" onClick={handleProgressClick}>
              <div 
            ...
            <div className="flex justify-between text-[11px] text-zinc-500 mt-2 font-bold tracking-tight">
          ...
          <div className="flex items-center justify-center gap-10 w-full mb-4">
            <button onClick={prevSong} className="text-zinc-400 hover:text-white transition-all active:scale-90">
              <SkipBack size={32} fill="currentColor" />
            </button>
            
            <button 
              onClick={togglePlay} 
              className="w-16 h-16 bg-white text-black rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-all active:scale-95"
            >
              {isPlaying ? <Pause size={30} fill="black" /> : <Play size={30} fill="black" className="ml-1" />}
            </button>

            <button onClick={nextSong} className="text-zinc-400 hover:text-white transition-all active:scale-90">
