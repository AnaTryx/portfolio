...
const StatusBar = () => {
  ...
  const getBatteryIcon = () => {
    if (battery.charging) return <BatteryCharging size={16} className="text-green-600" />;
    if (battery.level <= 20) return <BatteryLow size={16} className="text-red-500" />;
    if (battery.level <= 60) return <BatteryMedium size={16} className="text-slate-900" />;
    return <BatteryFull size={16} className="text-slate-900" />;
  };

  return (
    <div className="fixed top-0 left-0 w-full px-6 py-3 flex justify-between items-start z-[100] pointer-events-none select-none">
      {/* Lado Izquierdo: Recuperamos tu tipografía original */}
      <div className="flex flex-col items-start leading-tight">
        <span className="text-[14px] font-bold text-slate-900">{formattedTime}</span>
        <span className="text-[11px] font-medium text-slate-700 capitalize">
          {formattedDate.replace('.', '')}
        </span>
      </div>

      {/* Lado Derecho: Iconos modernos con Lucide */}
      <div className="flex gap-4 items-center pt-1">
        <div className="flex items-center gap-2 text-slate-900">
          <Plane size={14} />
          <WifiOff size={14} className="opacity-30" />
        </div>
